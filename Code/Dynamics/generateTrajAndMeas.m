...
%     % add error
    biasDrift = mvnrnd(zeros(size(params.imuBias.mean)),params.imuBias.Sigma);biasDrift = biasDrift(:);
    
    bias = bias + params.dt*biasDrift;
    
    Xtruth(ii,8:10) = bias;
    
    deltaV(:,ii) = deltaVTrue(:,ii) + bias;
        
    vel = X(ii-1,4:6) + deltaV(:,ii)';
    pos = X(ii-1,1:3) + vel*params.dt + 0.5*deltaV(:,ii)'*params.dt;
    
    X(ii,1:7) = [pos,vel,params.mu];
    X(ii,8:10) = params.imuBias.mean;
end
